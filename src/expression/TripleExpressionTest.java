...
public class TripleExpressionTest extends ExpressionTest {
	...
	protected void test() {
		...
		testExpression("x*y+(z-1)/10",
				new Add(new Multiply(new Variable("x"), new Variable("y")),
						new Divide(new Subtract(new Variable("z"), new Const(1)), new Const(10))),
				(x, y, z) -> x * y + (z - 1) / 10);
	}

	private static void testExpression(final String description, final TripleExpression actual,
			final TripleExpression expected) {
		System.out.println("Testing " + description);
		for (int i = 0; i < 10; i++) {
			for (int j = 0; j < 10; j++) {
				for (int k = 0; k < 10; k++) {
					assertEquals(String.format("f(%d, %d, %d)", i, j, k), expected.evaluate(i, j, k),
							actual.evaluate(i, j, k));
				}
			}
		}
	}
